...
開発・検証・本番は、`.env.dev`／`.env.stg`／`.env.prod`、Compose project name、network、
...
提供します。開発・検証・本番はそれぞれ異なるCompose project、container、data volumeへ分離します。
各runtimeは内部service名`mysql:3306`へ接続し、
`MYSQL_DATABASE`、`MYSQL_USER`、`MYSQL_PASSWORD`、`MYSQL_ROOT_PASSWORD`は秘密管理設定から
注入します。MySQLの初期化は空のvolumeでのみ行い、既存volumeを初期化しません。

検証runtimeはstaging projectで次のように起動します。

```bash
docker compose -p voxelsteward-stg --env-file .env.stg \
  -f compose.yaml -f compose.mysql.yaml -f compose.stg.yaml up -d --no-deps runtime
```

検証runtimeは`.env.stg`で指定したdatabase・userへ接続します。開発runtimeと同時に
...
`compose.mysql.yaml`と`compose.stg.yaml`はstagingの`runtime`へ重ねる環境専用overlayです。`BOT_MODE=normal`、
...
```bash
docker compose -p voxelsteward-stg --env-file .env.stg -f compose.yaml -f compose.mysql.yaml -f compose.stg.yaml up -d --no-deps runtime
docker compose -p voxelsteward-stg --env-file .env.stg -f compose.yaml -f compose.mysql.yaml -f compose.stg.yaml logs --no-log-prefix runtime
docker compose -p voxelsteward-stg --env-file .env.stg -f compose.yaml -f compose.mysql.yaml -f compose.stg.yaml stop runtime
docker compose -p voxelsteward-stg --env-file .env.stg -f compose.yaml -f compose.mysql.yaml -f compose.stg.yaml ps runtime
```
