...
MySQL実行基盤はIssue #37で管理します。開発・ネットワーク検証・本番は同じ固定digestの
共通service定義を使いますが、Compose projectとデータvolumeは分離します。同一volumeの共有や
既存本番データの移行は行わず、各環境のdatabaseと資格情報を明示的に設定します。
