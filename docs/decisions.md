...
- 決定時点の旧構成記録: `compose.verification.yaml`を`compose.yaml`へ重ね、既存`runtime`の`BOT_MODE=normal`、
...
- 決定: 基底`compose.yaml`から固定`.env`の`env_file`を除去し、`compose.dev.yaml`と`compose.prod.yaml`が
  現行は`.env.dev`／`.env.stg`／`.env.prod`を環境ごとに明示します。起動コマンドは`-p`でprojectを
  分け、認証volumeは`voxel-steward-dev-auth-*`／`voxel-steward-stg-auth-*`／`voxel-steward-prod-auth-*`へ分離します。固定
